...
test('feedback self-test Woo order skips confirmation send and records skip status', async () => {
  ...
  assert.deepEqual(
    {
      orderId: store.getOrder('10201')?.orderId,
      feedbackState: store.getOrder('10201')?.feedbackState,
      feedbackToken: store.getOrder('10201')?.feedbackToken,
      feedbackTestPhone: store.getOrder('10201')?.feedbackTestPhone,
      feedbackTestRunId: store.getOrder('10201')?.feedbackTestRunId
    },
    {
      orderId: '10201',
      feedbackState: 'waiting_for_feedback',
      feedbackToken: 'FDBK-TEST-20260420T102451Z-BTIAIB',
      feedbackTestPhone: '+491729031097',
      feedbackTestRunId: 'selftest-20260420T102451Z-btiaib'
    }
  );
});
...
test('tokenless text feedback matches production feedback candidate by phone and writes Woo feedback meta', async () => {
  ...
  listedOrders.push({
    ...
    line_items: [{ name: 'Produit Phone Fallback', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback'
    })
  });
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-text-phone-9011'
    },
    payload: {
      key: {
        cleanedSenderPn: '212611111911',
        fromMe: false,
        id: 'feedback-text-phone-9011-message'
      },
      messageBody: 'great service'
    }
  });
...
test('tokenless self-test text reply matches newest local self-test candidate by phone', async () => {
  const { app, listedOrders, confirmationService, wasenderCalls, wooNoteCalls, store } = createTestContext();
  const payload = {
    id: 9112,
    status: 'completed',
    total: '0.00',
    ...
    line_items: [{ name: 'Feedback Local Self Test', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback',
      token: 'FDBK-TEST-LOCAL-9112',
      testPhone: '+491729031097',
      testActive: 'yes',
      isTest: 'yes',
      testRunId: 'selftest-local-9112',
      requestedAt: '2026-04-20T13:48:58Z',
      sentAt: '2026-04-20T13:48:59Z'
    })
  };
  listedOrders.push(payload);
  ...
  await dispatch(app, {
    ...
    headers: {
      'x-wc-webhook-signature': signWoo(payload),
      'x-wc-webhook-delivery-id': 'delivery-self-test-local-9112'
    },
    payload
  });

  confirmationService.wooClient.listOrders = async () => {
    throw new Error('should not need Woo recovery when local self-test cache exists');
  };
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-self-test-local-9112'
    },
    payload: {
      key: {
        cleanedSenderPn: '491729031097',
        fromMe: false,
        id: 'feedback-self-test-local-9112-message'
      },
      messageBody: 'looks good without token'
    }
  });
  ...
  assert.equal(result.body.feedback, 1);
  assert.equal(wasenderCalls.length, 0);
  assert.equal(store.getOrder('9112').feedbackState, 'reply_received');
  assert.ok(wooNoteCalls.some((note) => note.orderId === '9112' && /\[text\]/.test(note.note)));
  const order = listedOrders.find((item) => String(item.id) === '9112');
  assert.equal(workflowMetaValue(order.meta_data, 'rhymat_feedback_state'), 'reply_received');
  assert.equal(workflowMetaValue(order.meta_data, 'rhymat_feedback_last_text'), 'looks good without token');
});

test('tokenless self-test reply picks newest active self-test candidate for the same phone', async () => {
  const { app, listedOrders, wooNoteCalls } = createTestContext();
  const olderOrder = {
    id: 9113,
    status: 'completed',
    ...
    line_items: [{ name: 'Feedback Older Self Test', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback',
      token: 'FDBK-TEST-OLDER-9113',
      testPhone: '+491729031097',
      testActive: 'yes',
      testRunId: 'selftest-older-9113',
      requestedAt: '2026-04-20T13:45:00Z',
      sentAt: '2026-04-20T13:45:01Z'
    })
  };
  const newerOrder = {
    id: 9114,
    ...
    line_items: [{ name: 'Feedback Newer Self Test', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback',
      token: 'FDBK-TEST-NEWER-9114',
      testPhone: '+491729031097',
      testActive: 'yes',
      testRunId: 'selftest-newer-9114',
      requestedAt: '2026-04-20T13:48:00Z',
      sentAt: '2026-04-20T13:48:59Z'
    })
  };
  listedOrders.push(olderOrder, newerOrder);

  for (const payload of [olderOrder, newerOrder]) {
    await dispatch(app, {
      method: 'POST',
      url: '/webhooks/woocommerce',
      headers: {
        'x-wc-webhook-signature': signWoo(payload),
        'x-wc-webhook-delivery-id': `delivery-${payload.id}`
      },
      payload
    });
  }
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-self-test-newest-9114'
    },
    payload: {
      data: {
        messages: {
          key: {
            cleanedSenderPn: '491729031097',
            fromMe: false,
            id: 'feedback-self-test-newest-9114-message'
          },
          message: {
            imageMessage: {
              mimetype: 'image/jpeg',
              url: 'https://cdn.example.com/9114.jpg'
            }
          }
        }
      }
  ...
  assert.equal(result.statusCode, 200);
  assert.equal(result.body.feedback, 1);
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '9113').meta_data, 'rhymat_feedback_state'), 'waiting_for_feedback');
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '9114').meta_data, 'rhymat_feedback_state'), 'reply_received');
  assert.ok(wooNoteCalls.some((note) => note.orderId === '9114' && /\[image\]/.test(note.note)));
});
...
test('media-only self-test reply without token matches via Woo recovery', async () => {
  ...
  assert.equal(result.body.feedback, 1);
  assert.equal(wasenderCalls.length, 0);
  assert.ok(wooNoteCalls.some((note) => note.orderId === '9031' && /\[image\]/.test(note.note)));
...
for (const scenario of [
  {
    ...
    payload: {
      data: {
        messages: {
          key: {
            cleanedSenderPn: '212611119033',
            fromMe: false,
            id: 'feedback-phone-audio-9033-message'
          },
          message: {
            audioMessage: {
              mimetype: 'audio/ogg',
              url: 'https://cdn.example.com/9033.ogg'
            }
          }
        }
      }
  ...
  },
  {
    ...
    payload: {
      data: {
        messages: {
          key: {
            cleanedSenderPn: '212611119034',
            fromMe: false,
            id: 'feedback-phone-document-9034-message'
          },
          message: {
            documentMessage: {
              mimetype: 'application/pdf',
              fileName: 'feedback-9034.pdf'
            }
          }
        }
      }
  ...
  test(`tokenless ${scenario.name} feedback matches production candidate by phone`, async () => {
    ...
    const result = await dispatch(app, {
      ...
      headers: {
        'x-wasender-signature': signWasenderPlain(),
        'x-event-id': `feedback-phone-${scenario.name}-${scenario.orderId}`
      },
...
test('tokenless self-test text reply matches via Woo recovery and does not trigger clarification message', async () => {
  const { app, listedOrders, wasenderCalls } = createTestContext();
  listedOrders.push({
    id: 9032,
    status: 'cancelled',
...
test('tokenless production feedback picks newest active production candidate for the same phone', async () => {
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-phone-ambiguous-9035'
    },
    payload: {
      key: {
        cleanedSenderPn: '212611119035',
        fromMe: false,
        id: 'feedback-phone-ambiguous-9035-message'
      },
...
test('tokenless self-test recovery uses bounded Woo paging and caches the matched order locally', async () => {
  ...
  listedOrders.push({
    ...
    line_items: [{ name: 'Feedback Recovered Self Test', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback',
      token: 'FDBK-TEST-9036',
      testPhone: '+491729031097',
      testActive: 'yes',
      testRunId: 'selftest-9036',
      requestedAt: '2026-04-20T13:48:58Z',
      sentAt: '2026-04-20T13:48:59Z'
    })
  });
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-self-test-recovery-9036'
    },
    payload: {
      key: {
        cleanedSenderPn: '491729031097',
        fromMe: false,
        id: 'feedback-self-test-recovery-9036-message'
      },
      messageBody: 'recovered without token'
    }
  });
...
test('tokenless feedback image without self-test context is ignored safely', async () => {
  const { app, store, wooNoteCalls } = createTestContext();
  const result = await dispatch(app, {
    method: 'POST',
    url: '/webhooks/wasender',
    headers: {
...
test('tokenless self-test image matches newest active self-test without falling into confirmation', async () => {
  ...
  listedOrders.push({
    ...
    line_items: [{ name: 'Produit Image Self Test', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback',
      token: 'FDBK-TEST-9111',
      testPhone: '+491729031097',
      testActive: 'yes',
  ...
  assert.equal(result.body.feedback, 1);
  assert.equal(store.read().events.at(-1).status, 'feedback_reply_received');
  assert.ok(wooNoteCalls.some((note) => note.orderId === '9111' && /\[image\]/.test(note.note)));
  assert.equal(wasenderCalls.length, 0);
  const order = listedOrders.find((item) => String(item.id) === '9111');
  assert.equal(workflowMetaValue(order.meta_data, 'rhymat_feedback_state'), 'reply_received');
});

test('tokenless self-test feedback no longer takes priority over confirmation replies for the same sender', async () => {
  ...
  assert.equal(result.body.processed, 1);
  assert.equal(result.body.confirmation, 1);
  assert.equal(store.read().events.at(-1).status, 'confirmed');
  assert.equal(wasenderCalls.length, 4);
  assert.equal(store.getOrder('913').confirmationState, 'confirmed');
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '914').meta_data, 'rhymat_feedback_state'), 'waiting_for_feedback');
});

test('tokenless cancelled self-test feedback no longer takes priority over real pending confirmation orders', async () => {
  const { app, store, listedOrders, wasenderCalls } = createTestContext();
  const confirmationOrder = {
    id: 915,
    status: 'pending',
...
test('plain confirmation reply wins over tokenless real feedback phone fallback', async () => {
  const { app, store, listedOrders, wasenderCalls } = createTestContext();
  const confirmationOrder = {
    id: 917,
    status: 'pending',
    ...
    line_items: [{ name: 'Produit Confirm Wins', quantity: 1 }]
  };
  ...
  await dispatch(app, {
    ...
    headers: {
      'x-wc-webhook-signature': signWoo(confirmationOrder),
      'x-wc-webhook-delivery-id': 'delivery-feedback-priority-917'
    },
    payload: confirmationOrder
  });
  ...
  listedOrders.push({
    ...
    line_items: [{ name: 'Produit Feedback Fallback', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback'
    })
  });
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-priority-917'
    },
    payload: {
      key: {
        cleanedSenderPn: '212611119170',
        fromMe: false,
        id: 'feedback-priority-917-message'
      },
      messageBody: '1'
    }
  });
  ...
  assert.equal(result.statusCode, 200);
  assert.equal(result.body.confirmation, 1);
  assert.equal(result.body.feedback, 0);
  assert.equal(store.getOrder('917').confirmationState, 'confirmed');
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '918').meta_data, 'rhymat_feedback_state'), 'waiting_for_feedback');
  assert.equal(wasenderCalls.length, 4);
});

test('explicit feedback token wins over pending confirmation order for the same sender', async () => {
  const { app, store, listedOrders, wasenderCalls, wooNoteCalls } = createTestContext();
  const confirmationOrder = {
    id: 919,
    status: 'pending',
    ...
    line_items: [{ name: 'Produit Pending Token', quantity: 1 }]
  };
  ...
  await dispatch(app, {
    ...
    headers: {
      'x-wc-webhook-signature': signWoo(confirmationOrder),
      'x-wc-webhook-delivery-id': 'delivery-feedback-token-919'
    },
    payload: confirmationOrder
  });
  ...
  listedOrders.push({
    ...
    line_items: [{ name: 'Produit Feedback Token', quantity: 1 }],
    meta_data: buildFeedbackMeta({
      state: 'waiting_for_feedback'
    })
  });
  ...
  const result = await dispatch(app, {
    ...
    headers: {
      'x-wasender-signature': signWasenderPlain(),
      'x-event-id': 'feedback-token-920'
    },
    payload: {
      key: {
        cleanedSenderPn: '212611119190',
        fromMe: false,
        id: 'feedback-token-920-message'
      },
      messageBody: 'FDBK-920 packed great'
    }
  });
  ...
  assert.equal(result.body.feedback, 1);
  assert.equal(result.body.confirmation, 0);
  assert.equal(store.getOrder('919').confirmationState, 'pending_confirmation');
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '920').meta_data, 'rhymat_feedback_state'), 'reply_received');
  assert.equal(workflowMetaValue(listedOrders.find((item) => String(item.id) === '920').meta_data, 'rhymat_feedback_last_text'), 'FDBK-920 packed great');
  assert.equal(wasenderCalls.length, 1);
  assert.ok(wooNoteCalls.some((note) => note.orderId === '920' && /\[text\]/.test(note.note)));
});
